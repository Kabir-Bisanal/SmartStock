# SmartStock Stage 10 — Inventory Optimization & Reorder Decision Engine

## Executive Summary

Stage 10 converted the frozen Version 1 demand forecasts into inventory decisions for all **300 item-store series**. The engine produced lead-time demand, safety stock, reorder points, target stock levels, stockout-risk estimates, service-level reorder quantities, cost-aware order quantities, stock statuses, and transparent priority scores.

The underlying M5 demand is real public competition data. The inventory balances, lead times, service levels, and costs are **synthetic demo assumptions**, not Walmart operational data. Under this demo snapshot, **198 series require a service-level order**, totaling **4,143 units**. The scenario-based cost optimizer reduced estimated planning cost from **16,937.5** to **4,892.6 demo currency units**, an estimated saving of **12,044.8**.
...
The Stage 9 evaluation artifact remains frozen through **2016-04-22**. A separate deployment artifact was refreshed through **2016-05-22** using the exact frozen `mean_28` method. It produced **9,000 daily forecasts from 2016-05-23 through 2016-06-21**.
...
Calibration used **27,000 mean-28 predictions** covering 2016-01-24 through 2016-04-22. The locked final-test period contributed **zero** calibration rows. Each series had sufficient validation history for item-store calibration, so all 300 series used their own residual mean and daily residual standard deviation.
...
The default review period is 7 days. The target covers demand until the next review opportunity as well as supplier lead time.
...
For each series, 300 deterministic demand scenarios were generated with seed 42 and validation-calibrated residual uncertainty. Daily scenario demand was clipped at zero. A bounded grid included zero, the service-level recommendation, and quantities covering the upper scenario-demand range.
...
The snapshot contains exactly 300 unique item-store series. Planned seeded profiles were:

| Predefined profile | Series |
| --- | --- |
| critical | 45 |
| healthy | 90 |
| low | 30 |
...
- Service-level reorders: **198 series / 4,143 units**
- Cost-optimized orders: **136 series / 5,181 units**
- Expected shortage without ordering: **3,312.6 units**
- Expected shortage after cost-optimized orders: **167.0 units**
- Expected excess without ordering: **2,358.3 units**
- Estimated cost saving: **12,044.8 demo currency units**

## Store-Level Results

| Store | Series | Reorders | Service units | Shortage before | Cost savings |
| --- | --- | --- | --- | --- | --- |
| CA_1 | 100 | 64 | 1,528 | 1,202.5 | 4,499.2 |
| TX_2 | 100 | 63 | 1,348 | 1,146.7 | 4,376.1 |
| WI_3 | 100 | 71 | 1,267 | 963.3 | 3,169.5 |

## Department-Level Results

| Department | Series | Reorders | Service units | Shortage before | Cost savings |
| --- | --- | --- | --- | --- | --- |
| FOODS_1 | 45 | 22 | 465 | 309.5 | 1,012.3 |
| FOODS_2 | 84 | 50 | 947 | 874.4 | 3,201.9 |
| FOODS_3 | 171 | 126 | 2,731 | 2,128.7 | 7,830.7 |

## Cost / Shortage Results

The cost-aware optimizer reduced expected shortage by **3,145.6 units** in the simulated planning horizons. The service-level and cost-optimal order quantities are deliberately both retained: the first prioritizes the configured service target, while the second explicitly trades shortage cost, holding cost, and fixed ordering cost.
